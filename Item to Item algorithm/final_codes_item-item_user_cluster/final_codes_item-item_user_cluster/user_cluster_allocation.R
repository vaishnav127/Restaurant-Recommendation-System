my_data1<-read.csv("Vash_FC.csv")   #new user
my_data1<-my_data1[, 2:25]
library(varhandle)
############## FILE INPUT  #######################3
my_data3<-read.csv("user_cluster_centroid.csv")
my_data3_subset<-my_data3[, 1:24]     #selecting the attributes similarity depends on
my_data1_subset<-my_data1
library(varhandle)
cluster_id<-my_data3_subset[, 1]
my_matrix<-(matrix(, nrow = length(cluster_id), ncol = nrow(my_data1)))
colnames(my_matrix)<-my_data1$business_id
row.names(my_matrix)<-cluster_id
my_data1_subset_cuisines<-my_data1_subset[,24]
my_data3_subset_cuisines<-my_data3_subset[,24]

library(varhandle)
my_data3_subset[,3]<-unfactor(my_data3_subset[,3])
my_data3_subset[,2]<-unfactor(my_data3_subset[,2])
my_data3_subset[,4]<-unfactor(my_data3_subset[,4])
my_data3_subset[,5]<-unfactor(my_data3_subset[,5])
my_data3_subset[,6]<-unfactor(my_data3_subset[,6])
my_data3_subset[,8]<-unfactor(my_data3_subset[,8])
my_data3_subset[,10]<-unfactor(my_data3_subset[,10])
my_data3_subset[,11]<-unfactor(my_data3_subset[,11])
my_data3_subset[,12]<-unfactor(my_data3_subset[,12])
my_data3_subset[,13]<-unfactor(my_data3_subset[,13])
my_data3_subset[,16]<-unfactor(my_data3_subset[,16])
my_data3_subset[,15]<-unfactor(my_data3_subset[,15])
my_data3_subset[,17]<-unfactor(my_data3_subset[,17])
my_data3_subset[,18]<-unfactor(my_data3_subset[,18])
my_data3_subset[,19]<-unfactor(my_data3_subset[,19])
my_data3_subset[,20]<-unfactor(my_data3_subset[,20])
my_data3_subset[,21]<-unfactor(my_data3_subset[,21])
my_data3_subset[,22]<-unfactor(my_data3_subset[,22])
my_data3_subset[,23]<-unfactor(my_data3_subset[,23])
my_data3_subset[,7]<-unfactor(my_data3_subset[,7])
my_data3_subset[,9]<-unfactor(my_data3_subset[,9])
my_data3_subset[,14]<-unfactor(my_data3_subset[,14])
...
for(l in 1:length(cluster_id))   #making a vector for each row
{print(l)
  i=l
  ...
  {print(j)
    ...
    print(".............................")
    my_matrix[my_data3_subset[i,"cluster_no"], my_data1_subset[j,"business_id"]]<-cosine(vec1,vec2)[1,1]
...
write.csv(main_matrix, "allocated_clusters.csv")